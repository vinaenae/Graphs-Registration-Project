...
class Graph:
    ...
    def has_cycle(self):
        ...
        for vertex, _ in enumerate(self.vertices):
            if vertex not in visited_set:
                ...
                stack.push(vertex)
                temporary_iteration.append(vertex)
                while not stack.is_empty():
                    current = stack.pop()
                    if current not in visited_set:
                        visited_set.add(current)
                        for neighbor_vertex, _ in enumerate(self.vertices):
                            if self.adjacency_matrix[current][neighbor_vertex] == 1:
                                if neighbor_vertex in temporary_iteration:
                                    return True
                                if neighbor_vertex not in visited_set:
                                    temporary_iteration.append(neighbor_vertex)
                                    stack.push(neighbor_vertex)
                        temporary_iteration.remove(current)
        return False
    ...
    def get_registration_plan(self):
        ...
        counts = {}
        for i, _ in enumerate(self.adjacency_matrix):
            for c, _ in enumerate(self.adjacency_matrix):
                if self.adjacency_matrix[i][c]:
                    if c not in counts:
                        counts[c] = 1
                    else:
                        counts[c] += 1
        ...
        for key, value in counts.items():
            if value == 0:
                registration_courses.append(key)

        for i in range(len(registration_courses)):
            v_depth = compute_depth(registration_courses[i])
            heap.insert(v_depth)
    ...
    def valid_path(n, edges, source, destination):
        ...
        for i in range(len(edges)):
            for j in range(len(edges[i])):
                if edges[i][j] not in adjacency_list:
                    adjacency_list[edges[i][j]] = []
        ...
        while not queue1.is_empty():
            current_v = queue1.dequeue()
            if current_v == destination:
                return True 
            for neighbor in adjacency_list[current_v]:
