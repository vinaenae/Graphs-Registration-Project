...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs(v):
            ...
            for neighbor in self.get_adjacent_vertices(v):
                depth_neighbor = dfs(neighbor)
                if depth_neighbor + 1 > depth_peak:
                    depth_peak = depth_neighbor + 1

            self.vertices[v].depth = depth_peak
            return depth_peak
    ...
    def has_cycle(self):
        ...
        temporary_stack =  []
        visited = set()
        def helper_function(vertex):
            temporary_stack.append(vertex)
            stack.push(vertex)
            for neighbor, _ in enumerate(self.vertices):
                if self.adjacency_matrix[vertex][neighbor]:
                    if neighbor in temporary_stack:
                        return True
                    else:
                        if neighbor not in visited:
                            if helper_function(neighbor):
                                cycle = True
                                return cycle
            stack.pop()
            visited.add(vertex)
            temporary_stack.remove(vertex)
            return False

        for vertex, _ in enumerate(self.vertices):
            if helper_function(vertex) and vertex not in visited:
                return True
        return False
    ...
    def get_registration_plan(self):
        ...
        number_prerequisites = [False] * len(self.vertices)

        for i in range(len(self.vertices)):
            for j in range(len(self.vertices)):
                if self.adjacency_matrix[i][j] == 1:
                    number_prerequisites[j] += 1

        take_upcoming = []
        for i in range(len(self.vertices)):
            if number_prerequisites[i] == 0:
                take_upcoming.append(i)
        ...
        while take_upcoming:
            ...
            for _ in range(min(4, len(take_upcoming))):
                max_depth = -1
                max_index = -1
                for i in range(len(take_upcoming)):
                    vertex = self.vertices[take_upcoming[i]]
                    if vertex.depth > max_depth or (vertex.depth == max_depth and vertex.label < self.vertices[max_index].label):
                        max_depth = vertex.depth
                        max_index = i

                course_index = take_upcoming.pop(max_index)
                overall_sem.append(self.vertices[course_index].label)

                for neighbor in self.get_adjacent_vertices(course_index):
                    number_prerequisites[neighbor] -= 1
                    if number_prerequisites[neighbor] == 0:
                        incoming.append(neighbor)

            result.append(overall_sem)
            take_upcoming.extend(incoming)
...
def main():
    ...
    for count_edges in range(num_edges):
        edge = input().strip().split()
        prerequisite_course, dependent_course = edge[0], edge[1]
        prereq_index = graph.get_index(prerequisite_course)
        dependent_index = graph.get_index(dependent_course)
        graph.add_edge(prereq_index, dependent_index)
